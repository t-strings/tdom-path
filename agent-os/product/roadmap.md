...
4. [ ] Relative Path Calculation — Implement relative path calculation based on render target. Convert Traversable
   full paths to relative paths from current page to asset location. Support site prefix for SSG deployments.

5. [ ] Build-Time Asset Collection — Implement asset collector that traverses Node tree to gather all static asset
...
6. [ ] Documentation and Examples — Write comprehensive API documentation with type signatures, create cookbook with
...
10. [ ] Static Analysis Integration — Create path validator that checks paths against filesystem at build time, implement
