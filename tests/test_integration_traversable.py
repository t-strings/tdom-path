...
def test_render_strategy_with_site_prefix():
    """Test custom render strategy with site prefix."""
    ...
    # Render with site prefix - verify it works without errors
    ...
    # Verify rendering produces a string href
    link = get_by_tag_name(rendered_tree, "link")
    assert isinstance(link.attrs["href"], str)
    # Verify path contains expected components (actual format may vary)
